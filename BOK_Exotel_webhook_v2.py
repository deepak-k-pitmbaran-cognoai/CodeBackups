"""
    This webhook contains custom changes for exotel.
    1. BOK Custom TnC Changes.
"""

from EasyChatApp.utils import check_and_send_broken_bot_mail, logger
...
# BOK Custom TnC Changes. >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
def check_TnC_consent_and_replace_message(profile: object, bot_obj:object, user_message: str, recommendations_after_language_change=[], selected_language="en") -> str:       
    """
        Args:
            profile (Profile): The user's profile object containing user-specific details, such as the last message date.
            bot_obj (Bot): The bot object representing the current bot instance interacting with the user.
            user_message (str): The current message sent by the user.

        Returns:
            user_message (str): The original user message or a replacement TnC message if consent is not recorded or if the last 
                message date is more than 24 hours ago. 
    """
    try:
        logger.info("====== Into check_TnC_consent_and_replace_message =======", extra=log_param)
        
        # translating user message into english language
        logger.info(f"selected_language: {selected_language}", extra=log_param)
        if selected_language != 'en':
            logger.info(f"Translating user_message into english language", extra=log_param)
            translated_user_message = translate_given_text_to_english(user_message)
        logger.info(f"user_message: {user_message}", extra=log_param)
        
        TNC_INTENT = "Terms and conditions"
        
        TNC_WHITELISTED_MSGS = ["disagree", "agree", "i do not agree", "i do agree"]

        last_message_date = profile.last_message_date
        logger.info(f"last_message_date: {last_message_date}", extra=log_param)
        
        time_24_hours_ago = (datetime.datetime.now() - datetime.timedelta(hours=24)).astimezone(last_message_date.tzinfo)
        logger.info(f"time_24_hours_ago: {time_24_hours_ago}", extra=log_param)

        # getting user consent recorded from data model
        data_obj = Data.objects.filter(bot=bot_obj, variable="is_TnC_agreed", user=profile).first()
        is_TnC_agreed = str(data_obj.get_value()).lower() if data_obj else "false"
        logger.info(f"is_TnC_agreed: {is_TnC_agreed}", extra=log_param)

        # replacing user_message with terms and conditions intent
        if ((last_message_date < time_24_hours_ago) or (is_TnC_agreed == "false")) and translated_user_message.lower() not in TNC_WHITELISTED_MSGS:
            
            # updating recommendations_after_language_change 
            if recommendations_after_language_change:
                recommendations_after_language_change = [TNC_INTENT]
                user_message = "Welcome to BOK customer service \n\nTo continue please provide your consent on our *'Terms and Conditions'*."
                logger.info("Lanuage change response text updated.", extra=log_param)
                logger.info("Returned from check_TnC_consent_and_replace_message. =======", extra=log_param)
                return user_message, recommendations_after_language_change
            
            # Translating the intent as per the selected language
            user_message = get_translated_text(
                TNC_INTENT, selected_language, EasyChatTranslationCache)
            profile.tree = None
            profile.save(update_fields=['tree'])
            
            data_obj.value = "false"
            data_obj.save(update_fields=['value'])
            
            logger.info("Message replaced with TnC intent.", extra=log_param)
        
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("Error in check_TnC_consent_and_replace_message: %s at %s",
...
def whatsapp_webhook(request_packet):
    ...
    #   GLOBAL CONFIGURATION VARIABLES:
        ...
        recommendations_after_language_change = ["Chat with Agent","Change Language", 'Finance', 'Visa', 'External Transfer', 'Electronic Services', 
            'Branches', 'Accounts']
        ...
        language_change_query_list = [
            "change language", "change the language", "language change", "switch language","switch languages", "alter language",
            "modify language","i want to change language to english","i want to change language to arabic","arabic language"]

        is_custom_TNC_enabled = True           # BOK Custom TnC Changes
        ...
        if is_change_language_triggered(user_id, default_bot_obj) and not user_obj.livechat_connected:
            ...
            try:
                ...
                if is_supported:
                    ...
                    text_message = "Welcome to BOK customer service \nPlease choose from the options below."

                    # check Tnc Message and replace message BOK Custom TnC Changes
                    if is_custom_TNC_enabled:
                        text_message, recommendations_after_language_change = check_TnC_consent_and_replace_message(user_obj, bot_obj, text_message, recommendations_after_language_change=recommendations_after_language_change, selected_language=selected_language)             # BOK Custom TnC Changes
                    

                    text_message = get_translated_text(
                        text_message, lang_obj.lang, EasyChatTranslationCache)
                    
                    logger.info(f"text_message ...... {text_message}", extra=log_param)
                    ...
                    if recommendations_after_language_change:
                        ...
                        if send_default_quick_reply or send_default_list_options:
                            ...
                            # if send_default_quick_reply == True:        [Product bug] Preventing to send quick replies, instead of checking false condition checking for true.
                            if send_default_quick_reply == False:
                                if total_options_count <= 3 and len(text_message) < 1024:
                                    logger.info("Checking to send quick reply buttons....", extra=log_param)
                                    if check_interative_capability_for_quick_reply(options=recommendations_after_language_change, options_type="recommendations"):
                                        logger.info("Sending quick replies....", extra=log_param)
                                        ...
                                        #                                              BUTTON_HEADER, message, BUTTON_FOOTER, mobile, menu_button_text=list_interactive_menu_button_text, extra="None")  [PRODUCT bug] after calling the quick reply API again calling the list message API.                                
                                ...
                                if total_options_count > 3 and total_options_count < 11 and len(text_message) < 1024:
                                    if check_interative_capability_for_list(options=recommendations_after_language_change, options_type="recommendations"):
                                        ...
                                        if LIST_HEADER_FOOTER_VISIBLE == False:
                                            BUTTON_HEADER = " "
                                            BUTTON_FOOTER = " "
#                                        logger.info("text_message: " + str(text_message) + "-------" + str(message), extra=log_param)
                                        # changing message argument to text_message for custom requirement                                            
                                        # is_send = sendWhatsAppInteractiveListMessage(exotel_apikey, exotel_apitoken, exotel_number, exotel_sid, exotel_subdomain, list_interactive_options_title, BUTTON_DISPLAY_NAMES,
                                        #                                              BUTTON_HEADER, message, BUTTON_FOOTER, mobile, menu_button_text=list_interactive_menu_button_text, extra="None")     # [PRODUCT Bug] instead of sending text message passing the message in the API
                                        is_send = sendWhatsAppInteractiveListMessage(exotel_apikey, exotel_apitoken, exotel_number, exotel_sid, exotel_subdomain, list_interactive_options_title, BUTTON_DISPLAY_NAMES,
                                                                                     BUTTON_HEADER, text_message, BUTTON_FOOTER, mobile, menu_button_text=list_interactive_menu_button_text, extra="None")
                                        empty_recommendations = True
                                        REVERSE_WHATSAPP_MESSAGE_DICT = TEMP_REVERSE_WHATSAPP_MESSAGE_DICT
    ...
    #   GET LANGUAGE SELECTED BY USER
        ...
        # check Tnc Message and replace message BOK Custom TnC Changes
        if is_custom_TNC_enabled:
            message, _ = check_TnC_consent_and_replace_message(user_obj[0], bot_obj, message, selected_language=selected_language)             # BOK Custom TnC Changes
    ...
    # Go Back Check
        logger.info(f"before go back check recommendations {recommendations}", extra=log_param)
        is_go_back_enabled = False
        if "is_go_back_enabled" in whatsapp_response["response"]:
          ...
          # [PRODUCT Bug] commented the below condition as we are getting go back text in recommendations from the execute query.  
            # if is_go_back_enabled:
            #     recommendations.append(go_back_text) 
     
        logger.info(f"after go back heck recommendations {recommendations}", extra=log_param)
    ...
    #   SEND SINGLE BUBBLE TEXT:
        logger.info(f"Check message {message}", extra=log_param)
        final_text_message = ""
    ...
    #   SENDING CHOICES AND RECOMMENDATIONS BOTH:
        if len(choice_str) > 0 and len(recommendation_str) > 0:
            mixed_choice = choice_str+"\n"+recommendation_str   # [PRODUCT bug] added \n in between the choice and recommendations as it is showing options in same line
            time.sleep(0.05)
