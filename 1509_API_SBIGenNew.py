...
def call_eClaimForm_API(processor_dict, **kwargs):
    try:
        ...
        url = processor_dict["API_REQUEST_PACKET"]["url"] = f"{SBIGEN_HOST}/custportal/v1/eclaim-form"

        headers = processor_dict["API_REQUEST_PACKET"]["headers"] = {
        ...
        payload = processor_dict["API_REQUEST_PACKET"]["payload"] = {
        ...
        processor_dict['print_payload'] = json.dumps(payload)
...
#        content = json.loads(decrypted_resp)
        ...
        raise Exception(f"Error in call_eClaimForm_API! {str(e)} at line no. {str(exc_tb.tb_lineno)}")
...
def f():
    ...
    try:
        ...
        response['claim_details'] = resp_text
        ...
        response['claim_details'] = 'We are not able to process your request due to connectivity failure. Please try again after sometime.'
        ...
        response['claim_details']='Some internal issues:'
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error('PostProcessorContent: %s at %s',str(E), str(exc_tb.tb_lineno))
        response['API_RESPONSE_PACKET']["Exception_error"] = 'Exception! - ' + str(E) + "at line no. " + str(exc_tb.tb_lineno)
        response['status_code'] = '500'
        response['status_message'] = 'ERROR :-  '+str(E)+ ' at line no: ' +str(exc_tb.tb_lineno)
        return response
